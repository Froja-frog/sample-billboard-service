...
class RegisterUserForm(forms.ModelForm):
    ...
    password_repeated = forms.CharField(label='Пароль (повторно)', widget=forms.PasswordInput, help_text='Введите тот же пароль еще раз для проверки')
    ...
    def clean(self):
        ...
        if password and password_repeated and password != password_repeated:
            errors = {'password_repeated': ValidationError("Введенные пароли не совпадают", code='password_mismatch')}
    ...
    class Meta:
        model = AdvUser
        fields = ('username', 'email', 'password', 'password_repeated', 'first_name', 'last_name', 'send_messages')
